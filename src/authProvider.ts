...
class GiteaPatSession implements AuthenticationSession {
	// We don't know the user's account name, so we'll just use a constant
	readonly account = { id: GiteaAuthenticationProvider.id, label: 'Personal Access Token' };
	// This id isn't used for anything in this example, so we set it to a constant
	readonly id = GiteaAuthenticationProvider.id;
...
export class GiteaAuthenticationProvider implements AuthenticationProvider, Disposable {
	static id = 'GiteaPAT';
	private static secretKey = 'GiteaPAT';
		...
		if (this.initializedDisposable === undefined) {
			...
			this.initializedDisposable = Disposable.from(
				...
				this.secretStorage.onDidChange(e => {
					if (e.key === GiteaAuthenticationProvider.secretKey) {
						void this.checkForUpdates();
					}
				}),
				// This fires when the user initiates a "silent" auth flow via the Accounts menu.
				authentication.onDidChangeSessions(e => {
					if (e.provider.id === GiteaAuthenticationProvider.id) {
						void this.checkForUpdates();
					}
				}),
			);
	...
	private cacheTokenFromStorage() {
		this.currentToken = this.secretStorage.get(GiteaAuthenticationProvider.secretKey) as Promise<string | undefined>;
	...
	async getSessions(_scopes?: string[]): Promise<readonly AuthenticationSession[]> {
		...
		return token ? [new GiteaPatSession(token)] : [];
	}
	...
	async createSession(_scopes: string[]): Promise<AuthenticationSession> {
		...
		const token = await window.showInputBox({
			...
			prompt: 'Enter an Gitea Personal Access Token (PAT).',
			password: true
		});
		...
		await this.secretStorage.store(GiteaAuthenticationProvider.secretKey, token);
		console.log('Successfully logged in to Gitea');

		return new GiteaPatSession(token);
	}

	// This function is called when the end user signs out of the account.
	async removeSession(_sessionId: string): Promise<void> {
		await this.secretStorage.delete(GiteaAuthenticationProvider.secretKey);
	}
}